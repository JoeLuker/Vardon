...
async function checkCharacterDevice() {
	...
	try {
		...
		const { data, error } = await supabase.from('game_character').select('id, name').limit(5);
      ...
      `;
			...
			if (charError) {
				...
				// Log abilities
				if (
					characterData.game_character_ability &&
					characterData.game_character_ability.length > 0
				) {
					console.log('\nAbilities:');
