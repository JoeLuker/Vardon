...
async function cleanupFeatureCode() {
	...
	if (fs.existsSync(FEATURES_DIR)) {
		...
		try {
			...
			subdirs.forEach((dir) => {
				...
				const files = fs.readdirSync(dirPath).filter((file) => file.endsWith('.ts')).length;
	...
	console.log('   ------------------');
	console.log(
		'   Since the migration is complete, you can optionally remove the migration utilities:'
	);
	...
	if (fs.existsSync(UI_DIR)) {
		try {
			const uiFiles = fs.readdirSync(UI_DIR).filter((file) => file.endsWith('.svelte'));
	...
	if (fs.existsSync(TEST_DIR)) {
		try {
			const testFiles = fs.readdirSync(TEST_DIR).filter((file) => file.endsWith('.ts'));
