...
async function exportTable(tableName) {
	...
	const { data: localData, error: localError } = await localSupabase.from(tableName).select('*');
	...
	const { error: deleteError } = await prodSupabase.from(tableName).delete().gte('id', 0); // Delete all records
	...
	for (let i = 0; i < localData.length; i += chunkSize) {
		...
		const { error: insertError } = await prodSupabase.from(tableName).insert(chunk);

		if (insertError) {
			console.error(
				`❌ Error inserting chunk ${i}-${i + chunk.length} into production ${tableName}:`,
				insertError
			);
			return false;
		}
		...
		process.stdout.write(
			`   Inserted ${Math.min(i + chunkSize, localData.length)}/${localData.length} records\r`
		);
	}
...
async function verifyConnection(client, name) {
	try {
		...
		console.log(
			`⚠️  ${name} connection test completed (${error.message || 'auth check complete'})`
		);
...
async function main() {
	...
	if (successCount === totalTables) {
		...
		for (const table of verifyTables) {
			const { data, error } = await prodSupabase.from(table).select('count(*)').single();
