...
async function queryDatabase() {
	try {
		...
		// Query abilities
		console.log('\n===== Abilities =====');
		const { data: abilities, error: abilitiesError } = await supabase.from('ability').select('*');
		...
		if (abilitiesError) {
			...
			if (abilities && abilities.length > 0) {
				...
				abilities.forEach((ability) => {
					console.log(
						`- ID: ${ability.id}, Name: ${ability.name}, Label: ${ability.label || ability.name}`
					);
				});
			} else {
		...
		// Query skills
		console.log('\n===== Skills =====');
		const { data: skills, error: skillsError } = await supabase.from('skill').select('*').limit(10);
