	...
	try {
		...
		const classes = [
			...
			},
			{
				id: 3,
				name: 'Rogue',
				description: 'Masters of stealth and skill',
				hit_die: 8,
		...
		const { data: classData, error: classError } = await prodSupabase.from('class').insert(classes);
		...
		const skills = [
			...
			},
			{
				id: 5,
				name: 'Spellcraft',
				description: 'Understanding magic',
				ability_id: 4,
				created_at: new Date().toISOString(),
				updated_at: new Date().toISOString()
			}
		...
		const { data: skillData, error: skillError } = await prodSupabase.from('skill').insert(skills);
		...
		const { data: verifyClasses } = await prodSupabase.from('class').select('name').limit(3);
