...
import {
	createPluginManager,
	type PluginManager
} from '../src/lib/domain/plugins/PluginManagerComposed';
...
function initializeAbilitiesFromCharacter(
	...
	if (
		rawCharacter.game_character_ability &&
...
export async function initializeApplication(
	options: { gameData?: any; debug?: boolean } = {}
): Promise<Application> {
	...
	const abilityCapability = createAbilityCapability(bonusCapabilityInstance, { debug });
	...
	async function loadCharacter(characterId: number): Promise<Entity | null> {
		try {
			...
			if (!createResult.success) {
				console.error(
					`Failed to create entity file: ${entityPath}, error: ${createResult.errorMessage}`
				);
				return null;
			}
			...
			try {
				...
				if (result === 0) {
					if (debug) {
						console.log(
							`Character ${characterId} loaded successfully: ${(updatedEntity as Entity).name}`
						);
					}
					return updatedEntity as Entity;
				} else {
					console.error(
						`Failed to read entity after initialization: ${entityPath}, error code: ${result}`
					);
					return null;
				}
			} finally {
