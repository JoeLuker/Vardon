<script lang="ts">
	import { getCharacter, updateExtract } from '$lib/state/characterStore';
	...
	let usedSlotsByLevel = $derived(() => {
		...
		Object.entries(extractsByLevel()).forEach(([levelStr, extracts]) => {
			...
			used[level] = extracts.filter(
				(extract: DatabaseCharacterExtract) => extract.prepared > 0 && extract.used > 0
			).length;
		});
		return used;
	});
...
<div class="card">
	<div class="mb-6 flex items-center justify-between">
		...
		<div class="flex items-center gap-2">
			...
			<button
				class="hover:text-primary-dark text-sm text-primary"
				onclick={() => {
			...
			<div class="py-4 first:pt-0 last:pb-0">
				...
				<div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
					{#each extracts as extract (extract.id)}
						<div
							class="group relative rounded-lg bg-white/50 p-3 shadow-sm
...
{#if showPrepareModal}
	...
	>
		<div class="card relative max-h-[90vh] w-full max-w-2xl overflow-auto">
			<h2 id="prepare-extracts-title" class="mb-4 text-xl font-bold">Prepare Extracts</h2>
			<!-- Add extract preparation UI here -->
			<div class="mt-4 flex justify-end gap-2">
				<button class="btn btn-secondary" onclick={() => (showPrepareModal = false)}>
					Cancel
				</button>
				<button class="btn" disabled={updateState.status === 'syncing'}> Prepare </button>
			</div>
		</div>
	</div>
{/if}
