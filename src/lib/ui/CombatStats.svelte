...
<script lang="ts">
	...
	let quickActions = $state([
		...
		},
		{
			...
			label: '+1',
			disabled: () =>
				resources.bombs.remaining >= resources.bombs.perDay || updateState.status === 'syncing'
		}
	]);
	...
	async function handleQuickUpdate(amount: number) {
		const newValue = Math.max(
			0,
			Math.min(resources.bombs.perDay, resources.bombs.remaining + amount)
		);
...
<div class="space-y-3 rounded border border-gray-200 bg-white p-4 text-sm">
	...
	<div class="flex items-center justify-between">
		<div class="flex items-center gap-2">
			...
			{#if isEditing}
				<input
					type="number"
					class="w-16 rounded border border-gray-300 px-1 text-center text-sm"
					value={inputValue}
					min="0"
			...
			{:else}
				<button
					class="rounded px-2 py-1 text-xl font-bold hover:bg-gray-100"
					onclick={() => {
						isEditing = true;
		...
		<div class="flex gap-1">
			{#each quickActions as { amount, label, disabled }}
				<button
					class="rounded border px-1 py-0.5 text-xs hover:bg-gray-100 disabled:opacity-50"
	...
	<div class="space-y-1">
		...
		<div class="flex items-center justify-between">
			...
			<div class="group relative text-lg font-bold">
				{combat.attacks.melee.bonus}
				<Tooltip
					text={`BAB (${stats.combat.baseAttackBonus}) + STR (${stats.attributes.modifiers.temporary.str}) + Other Mods`}
				/>
			</div>
		</div>
		{#if combat.attacks.melee.damage}
			<div class="text-right text-xs text-gray-500">Damage {combat.attacks.melee.damage}</div>
		{/if}
		...
		<div class="flex items-center justify-between">
			...
			<div class="group relative text-lg font-bold">
				...
				<Tooltip
					text={`BAB (${stats.combat.baseAttackBonus}) + DEX (${stats.attributes.modifiers.temporary.dex}) + Other Mods`}
				/>
			</div>
		</div>
		{#if combat.attacks.ranged.damage}
			<div class="text-right text-xs text-gray-500">Damage {combat.attacks.ranged.damage}</div>
		{/if}
	</div>
		...
		<div class="flex items-center justify-between text-center text-sm">
			{#each saveThrows as { label, value }}
				<div class="group relative flex-1">
					...
					<div class="text-lg font-bold">{value() > 0 ? '+' : ''}{value()}</div>
					<div class="text-xs text-gray-500">{label}</div>
