	...
	function getArchetypeClass(archetype: any) {
		...
		if (character?.game_character_class) {
			for (const charClass of character.game_character_class) {
				if (
					charClass.class?.class_feature?.some(
...
<Card.Root>
	<Card.Header>
		...
		<Card.Description>Class customization with specialized options</Card.Description>
	</Card.Header>
	<Card.Content>
		{#if !character}
				...
				{#each archetypes as archetype}
					<div class="rounded-md border p-4">
						<div class="mb-2 flex items-center justify-between">
							<h3 class="text-lg font-medium">{archetype.label || archetype.name}</h3>
						...
						{#if character.archetypeFeatures?.length}
							<h4 class="mb-2 mt-3 font-medium">Features</h4>
