<script lang="ts">
	import { getCharacter, updateConsumable } from '$lib/state/characterStore';
	...
	const quickActions = $state.raw([
		{
			amount: -1,
			label: '-1',
			getDisabled: (value: number) => value <= 0 || updateState.status === 'syncing'
		},
		{
			amount: 1,
			label: '+1',
			getDisabled: (value: number) => value >= 10 || updateState.status === 'syncing'
		}
	]);
</script>
...
<div class="space-y-3 rounded border border-gray-200 bg-white p-4 text-sm">
	...
	<div class="space-y-2">
		{#each consumables as { type, label, value, effect, maxStock }}
			<div
				class="rounded bg-gray-50 p-2 text-sm transition-colors hover:bg-gray-100"
				role="group"
			...
			>
				<div class="mb-1 flex items-center justify-between">
					...
					<div class="flex gap-1">
						{#each quickActions as { amount, label, getDisabled }}
							<button
								class="rounded border px-1 py-0.5 text-xs hover:bg-gray-200 disabled:opacity-50"
				...
				<div class="mb-1 flex items-center">
					{#if editingType === type}
						<input
							type="number"
							class="w-12 rounded border border-gray-300 px-1 text-center text-sm"
							value={inputValue}
							min="0"
							max={maxStock}
					...
					{:else}
						<button
							class="w-12 rounded py-1 text-center text-xl font-bold hover:bg-gray-200
					...
					{/if}
					<span class="ml-1 text-xs text-gray-600">/ {maxStock}</span>
				</div>

				<div class="mb-1 text-xs text-gray-600">{effect}</div>

				<button
					class="w-full rounded border py-1 text-center text-xs hover:bg-gray-200 disabled:opacity-50"
