<script lang="ts">
	...
	function formatDescription(description: string | null): {
		...
		// Uses per day/week
		const usesMatch =
			gift.match(/\b(\d+)(?:\/|\s+per\s+)(day|week)\b/i) ||
		...
		const affectsBloodConsumption =
			stain.includes('blood') &&
	...
	function getCorruptionData() {
		...
		return corruptions.map((corruption: any) => {
			...
			const corruptionManifestations = manifestations.filter((m: any) => {
	...
	function showManifestationDetail(manifestationEntry: any) {
		...
		if (manifestation?.prerequisites && manifestation.prerequisites.length > 0) {
			...
			manifestation.prerequisites.forEach(
				(prereq: {
					...
					if (prereq.prerequisite) {
						const name =
							prereq.prerequisite.label ||
							prereq.prerequisite.name ||
							`ID:${prereq.prerequisite_manifestation_id}`;
	...
	function buildPrerequisiteTree(manifestations: any[]) {
		...
		const manifestationMap = manifestations.reduce(
			(map, entry) => {
		...
		(Object.values(manifestationMap) as ManifestationNode[]).forEach((node) => {
			...
			const prerequisiteId =
				node.manifestation.prerequisite?.id || node.manifestation.prerequisite_manifestation_id;
	...
	function renderTreeNode(
		...
		return {
			...
			children: node.children.map((child: ManifestationNode, i: number) =>
				renderTreeNode(child, manifestationLevel, i === node.children.length - 1, `${path}-${i}`)
			)
		};
	}

	// Get prerequisite tree data for a corruption
	function getPrerequisiteTreeData(
		corruption: any,
		manifestations: any[]
	): {
		nodes: TreeNodeData[];
		...
		const nodes = rootNodes.map((node, i) =>
			renderTreeNode(node, corruption.manifestation_level || 0, i === rootNodes.length - 1, `${i}`)
		);
					...
					{#each corruptionData as { corruption, manifestations }}
						<div class="corruption-group">
							...
							{#if corruption.description}
								<p class="mb-3 text-sm text-muted-foreground">{corruption.description}</p>
							{/if}
							...
							{#if manifestations.length > 0}
								<Tabs.Root value="cards" class="mb-4">
									...
									<Tabs.Content value="cards">
										<div class="mt-4 grid gap-2 sm:grid-cols-1 sm:gap-3 md:grid-cols-2">
											{#each manifestations as manifestationEntry}
												{@const manifestation = getManifestationFromEntry(manifestationEntry)}
												{@const descriptionParts = formatDescription(manifestation?.description)}
												{@const available = isManifestationAvailable(
													manifestation,
												...
												>
													<div
														class="relative space-y-2 overflow-hidden rounded-lg border p-3 transition-colors hover:bg-muted/50"
														...
														<div class="flex flex-wrap items-center gap-2">
															...
															<div class="flex flex-wrap gap-1">
																...
																{#if manifestation?.prerequisite_manifestation_id}
																	<Badge
																		variant="outline"
																		class="bg-amber-100 text-xs dark:bg-amber-950"
																	>
															...
															<div class="text-xs text-muted-foreground">
																{#if descriptionParts.gift}
																	<div class="line-clamp-2">
																		<span class="text-success font-medium">Gift:</span>
																		{descriptionParts.gift}
																	</div>
																{/if}
																{#if descriptionParts.gift && descriptionParts.stain}
																	<div class="my-1"></div>
																...
																{#if descriptionParts.stain}
																	<div class="line-clamp-2">
																		<span class="font-medium text-destructive">Stain:</span>
																		{descriptionParts.stain}
																	</div>
																{/if}
															</div>
														{/if}
									...
									<Tabs.Content value="tree" class="mt-4">
										<div class="rounded-md border bg-background py-3 pl-5 pr-3">
											<!-- Tree Visualization -->
											{#if manifestations.length > 0}
												...
												{#if treeResult.nodes.length > 0}
													{#if treeResult.isFallbackTree}
														<div
															class="mb-3 rounded border border-amber-200 bg-amber-50 px-2 py-1 text-xs text-amber-600 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-400"
														>
															No direct prerequisite relationships found. Showing a suggested
															progression based on manifestation levels.
														</div>
													{/if}
												...
												{:else}
													<p class="text-center text-sm text-muted-foreground">
														No prerequisite relationships found. This may be because manifestations
														don't have prerequisite_manifestation_id or prerequisite object set.
													</p>
													<div class="mt-3 border-t pt-2 text-xs">
														...
														<ul class="mt-1 space-y-1 text-muted-foreground">
															{#each manifestations as manifestationEntry}
																{@const manifestation =
																	getManifestationFromEntry(manifestationEntry)}
																{#if manifestation}
																	<li>
																		<span class="font-medium"
																			>{manifestation.label || manifestation.name}</span
																		>
																		{#if manifestation.prerequisites && manifestation.prerequisites.length > 0}
																			<span class="text-emerald-600 dark:text-emerald-400">
																				- Has {manifestation.prerequisites.length} prerequisite(s):
																				{manifestation.prerequisites
																					.map(
																						(p: {
																							prerequisite_manifestation_id: number;
																							prerequisite?: { label?: string; name?: string };
																						}) =>
																							p.prerequisite?.label ||
																							p.prerequisite?.name ||
																							`ID:${p.prerequisite_manifestation_id}`
																					)
																					.join(', ')}
																			</span>
																		{:else if manifestation.prerequisite}
																			<span class="text-emerald-600 dark:text-emerald-400">
																				- Has prerequisite: {manifestation.prerequisite.label ||
																					manifestation.prerequisite.name}
																			</span>
																		{:else}
																			<span class="text-amber-600 dark:text-amber-400">
																				- No prerequisite</span
																			>
																		{/if}
																	</li>
																{/if}
		...
		>
			{#if selectedManifestationDetail}
				<Dialog.Header class="border-b bg-background p-6">
					...
					<Dialog.Description class="mt-2 flex flex-wrap items-center gap-2">
				...
				<div class="max-h-[60vh] overflow-y-auto p-6">
					<div class="prose prose-sm dark:prose-invert max-w-none space-y-4">
						...
						{#if selectedManifestationDetail.stain}
							<div>
								<h3 class="text-base font-medium text-destructive">Stain</h3>
								<p class="whitespace-pre-wrap">{selectedManifestationDetail.stain}</p>

								{#if selectedManifestationDetail.affects_blood_consumption}
									<div
										class="mt-3 rounded-md border border-red-200 bg-red-50 p-2 text-sm dark:border-red-800 dark:bg-red-950/40"
									>
										<span class="font-medium">Note:</span> This manifestation affects your blood consumption
										requirements.
									</div>
								{/if}
							</div>
