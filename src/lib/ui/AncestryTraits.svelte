    ...
    {:else}
      <div class="mb-4">
        ...
        {#if ancestry.description}
          <p class="text-sm text-muted-foreground mt-1">{ancestry.description}</p>
        {/if}
      </div>
          ...
          {#each ancestryTraits as trait}
            <div class="border rounded-lg p-4">
              ...
              {#if trait.description}
                <p class="text-sm mb-3">{trait.description}</p>
              {/if}
              ...
              {#if typeof trait.id === 'number'}
                ...
                {#if benefits.length > 0}
                  <div class="mt-2">
                    <h5 class="text-sm font-medium mb-1">Benefits:</h5>
                    <ul class="text-sm space-y-2">
                      {#each benefits as benefit}
                        <li class="pl-3 border-l-2 border-muted">
                              ...
                              {#each benefit.bonuses as bonus}
                                <li>
                                  {#if bonus.target_specifier}
                                    +{bonus.value || 0} {bonus.bonus_type?.name || 'untyped'} bonus to {bonus.target_specifier.name}
                                  {:else}
