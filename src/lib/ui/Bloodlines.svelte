<script lang="ts">
    ...
    function getSorcererLevel() {
        const sorcererClass = character?.game_character_class?.find(
            gc => gc.class?.name?.toLowerCase().includes('sorcerer')
        );
...
<Card.Root>
    <Card.Header>
        ...
        <Card.Description>
            Sorcerer bloodline powers and spells
        </Card.Description>
    </Card.Header>
    <Card.Content>
        {#if !character}
                ...
                {#each bloodlines as bloodline}
                    <div class="border rounded-lg p-4">
                        ...
                        {#if bloodline.description}
                            <p class="text-sm text-muted-foreground mb-4">{bloodline.description}</p>
                        {/if}
                        
                        {#if bloodline.spells && bloodline.spells.length > 0}
                            <h4 class="font-medium text-sm mb-2">Bloodline Spells</h4>
