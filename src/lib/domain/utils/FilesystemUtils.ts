...
export function ensureCharacterParentDirectoriesExist(
	...
	try {
		...
		if (!kernel.exists(PATHS.PROC_CHARACTER)) {
			...
			if (!result.success) {
				console.error(
					`[FilesystemUtils] Failed to create directory: ${PATHS.PROC_CHARACTER}`,
					result
				);
				return false;
			}
...
export function fixCharacterPath(
	...
	try {
		...
		if (stats?.isDirectory) {
			console.warn(
				`[FilesystemUtils] Found a directory at ${characterPath} when it should be a file. Fixing...`
			);
			...
			if (characterData) {
				...
				if (!createResult.success) {
					console.error(
						`[FilesystemUtils] Failed to create character file: ${createResult.errorMessage}`
					);
					return false;
				}
			}
