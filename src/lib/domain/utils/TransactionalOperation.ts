 ...
 */
export abstract class TransactionalOperation<TInput, TOutput> extends BaseIdempotentOperation<
	TInput,
	TOutput
> {
	 ...
	 */
	constructor(id: string, options: Partial<TransactionOptions> = {}, debug: boolean = false) {
		super(id, debug);
	...
	protected async doExecute(input: TInput): Promise<TransactionResult<TOutput>> {
		...
		try {
			...
			if (this.defaultOptions.transactionTimeout > 0) {
				return await this.withTimeout(transactionPromise, this.defaultOptions.transactionTimeout);
			} else {
	...
	private async executeTransaction(
		...
		try {
			...
			return {
				...
				preparationTimeMs: preparationEndTime - preparationStartTime,
				commitTimeMs:
					commitEndTime && commitStartTime ? commitEndTime - commitStartTime : undefined,
				transactionTimeMs: Date.now() - context.startTime,
				timestamp: Date.now()
			};
		} catch (error) {
			...
			if (this.defaultOptions.autoRollback && context.state === TransactionState.PREPARED) {
				try {
					rollbackStartTime = Date.now();
					await this.rollback(input, context);
					rollbackEndTime = Date.now();
			...
			return {
				...
				state: context.state,
				preparationTimeMs:
					preparationEndTime && preparationStartTime
						? preparationEndTime - preparationStartTime
						: undefined,
				commitTimeMs:
					commitEndTime && commitStartTime ? commitEndTime - commitStartTime : undefined,
				rollbackTimeMs:
					rollbackEndTime && rollbackStartTime ? rollbackEndTime - rollbackStartTime : undefined,
				transactionTimeMs: Date.now() - context.startTime,
				timestamp: Date.now()
			};
		}
...
export function withTransaction<TInput, TOutput>(
	...
	return async (input: TInput): Promise<TransactionResult<TOutput>> => {
		...
		try {
			...
			// Auto-rollback if enabled and prepared
			if (opts.autoRollback && context.state === TransactionState.PREPARED) {
				try {
					rollbackStartTime = Date.now();
					await rollbackFn(input, context);
					rollbackEndTime = Date.now();
