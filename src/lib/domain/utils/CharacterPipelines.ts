...
export function createCharacterToUIModelPipeline(
	...
	return (
		DataPipeline.create<CharacterEntity, CharacterUIModel>(kernel, {
			...
			.pipe(
				(data) => {
					...
					return {
						...
						calculatedAbilities: Object.entries(abilities(data) || {}).reduce(
							(acc, [key, value]) => {
								acc[key] = {
				...
				{
					name: 'extract_abilities',
					description: 'Extract and calculate ability scores and modifiers'
				}
			)
			// Extract skills
			.pipe(
			...
			.pipe(
				(data) => {
					...
					const processedFeatures = (entity) =>
						entity.capabilities?.classes?.processedClassFeatures || [];
...
export function createAbilityScorePipeline(
	...
	CharacterEntity,
	Record<string, { score: number; modifier: number; bonuses: any[] }>
> {
	return (
		DataPipeline.create(kernel, {
			...
			.pipe(
				(data) => {
					...
					for (const [key, value] of Object.entries(abilities)) {
						...
						// Sum all bonuses
						const totalBonus = Object.values(bonusByType).reduce(
							(sum, bonus: any) => sum + bonus.value,
							0
						);
				...
				},
				{
					...
					description: 'Calculate final ability scores and modifiers'
				}
			)
	);
}
		...
		DataPipeline.create(kernel, {
			...
			.pipe(
				(data) => {
					...
					const hasToughness = (data.entity.capabilities?.feats?.feats || []).some(
						(feat) => feat.name === 'Toughness'
					);
