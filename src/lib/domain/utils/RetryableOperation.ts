 ...
 */
export abstract class RetryableOperation<TInput, TOutput> extends BaseIdempotentOperation<TInput, TOutput> {
  ...
  protected async doExecute(input: TInput): Promise<RetryableResult<TOutput>> {
    ...
    while (true) {
      ...
      try {
        ...
        if (
          attempts <= this.retryOptions.maxRetries && 
          this.retryOptions.isRetryable(err)
        ) {
  ...
  private calculateDelay(attempt: number): number {
    ...
    const exponentialDelay = this.retryOptions.initialDelay * 
  ...
  return async (...args: TArgs): Promise<TResult> => {
    ...
    while (true) {
      ...
      try {
        ...
        // Check if we should retry
        if (
          attempts <= retryOpts.maxRetries && 
          retryOpts.isRetryable(err)
        ) {
          // Calculate delay for next retry
          const delay = retryOpts.initialDelay * 
            Math.pow(retryOpts.backoffFactor, attempts - 1);
