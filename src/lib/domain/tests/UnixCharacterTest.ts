...
export async function runUnixCharacterTest(): Promise<string> {
  ...
  try {
    ...
    const sampleData = SampleCharacters.getWarrior();
    
    // Create mock game data with the sample character
    const mockGameData = {
      gameAPI: {
    ...
    const entityPath = `/entity/character-1`;
    ...
    try {
      ...
      if ((entityData as any).id !== 'character-1') {
      ...
      await kernel.executePlugin('character-test-plugin', 'character-1', { newHp: 75 });
