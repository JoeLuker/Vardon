...
export async function runUnixArchitectureTest(): Promise<void> {
	...
	console.log(
		`Create /test directory: ${mkdirResult.success ? 'success' : mkdirResult.errorMessage}`
	);
	...
	console.log(
		`Device read result: ${deviceReadResult === ErrorCode.SUCCESS ? 'success' : deviceReadResult}`
	);
	...
	console.log(
		`Device write result: ${deviceWriteResult === ErrorCode.SUCCESS ? 'success' : deviceWriteResult}`
	);
