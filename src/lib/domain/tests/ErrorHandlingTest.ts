...
export async function testErrorHandling(): Promise<boolean> {
	...
	try {
		...
		const error = createError(ErrorCode.ENOENT, 'File not found', {
	...
	try {
		...
		if (successResult.errorCode !== ErrorCode.SUCCESS) {
			console.error(
				`ERROR: Success result should have errorCode=SUCCESS, got ${successResult.errorCode}`
			);
			testsPassed = false;
		}
		...
		const failureResult = failure<number>(ErrorCode.ENOENT, 'File not found', {
			component: 'TestComponent',
			operation: 'testOperation'
		});
		...
		if (failureResult.errorCode !== ErrorCode.ENOENT) {
			console.error(
				`ERROR: Failure result should have errorCode=ENOENT, got ${failureResult.errorCode}`
			);
			testsPassed = false;
		}
		...
		if (failureResult.errorMessage !== 'File not found') {
			console.error(
				`ERROR: Failure result should have errorMessage="File not found", got "${failureResult.errorMessage}"`
			);
			testsPassed = false;
		}
	...
	try {
		...
		if (!failureResult.errorMessage?.includes('Operation failed')) {
			console.error(
				`ERROR: withResource should include original error message, got "${failureResult.errorMessage}"`
			);
			testsPassed = false;
		}
	...
	try {
		...
		if (successResult.data !== 'Operation succeeded') {
			console.error(
				`ERROR: tryWithResources should return correct data, got "${successResult.data}"`
			);
			testsPassed = false;
		}
		...
		if (!resourceClosed) {
			console.error(
				'ERROR: tryWithResources should have called cleanup function even after failure'
			);
			testsPassed = false;
		}
	...
	try {
		...
		if (sysError.code !== ErrorCode.EIO) {
			console.error(
				`ERROR: Logger error method should return SystemError with default EIO code, got ${sysError.code}`
			);
			testsPassed = false;
		}
		...
		if (sysError.context.component !== 'TestComponent') {
			console.error(
				`ERROR: Logger error method should set component name, got ${sysError.context.component}`
			);
			testsPassed = false;
		}
		...
		if (sysError.context.operation !== 'testOperation') {
			console.error(
				`ERROR: Logger error method should pass operation context, got ${sysError.context.operation}`
			);
			testsPassed = false;
		}
...
export async function testWithFile(): Promise<boolean> {
	...
	try {
		...
		// Test successful operation
		const result = await withFile<string>(mockKernel, '/test/path', 'r', (fd) => {
			if (fd !== 42) {
...
export async function runErrorHandlingTests(): Promise<boolean> {
	...
	const results = await Promise.all([testErrorHandling(), testWithFile()]);
