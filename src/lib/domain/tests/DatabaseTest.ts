...
async function loadCharactersFromDatabase(gameAPI: GameRulesAPI): Promise<CompleteCharacter[]> {
  ...
  const characterPromises = characterData.map(char => 
    gameAPI.getCompleteCharacterData(char.id)
  );
...
function checkAbilities(
  ...
  for (const abilityData of character.game_character_ability) {
    ...
    if (assembled[abilityName as keyof AssembledCharacter]) {
      ...
      console.log(`  ${abilityName}: DB=${abilityValue}, Assembled=${assembledAbility.total}, Entity=${entityAbility}`);
...
function checkSkills(
  ...
  for (const [skillId, ranks] of skillRanks.entries()) {
    if (assembled.skills[skillId]) {
      const skillName = character.game_character_skill_rank?.find(
        rank => rank.skill_id === skillId
      )?.skill?.name || `Skill ${skillId}`;
      
      console.log(`  ${skillName} (ID: ${skillId}): Ranks=${ranks}, Entity ranks=${entity.character.skills[skillName.toLowerCase()]?.ranks || 0}`);
...
function checkFeatures(
  ...
  console.log(`  Expected features: ${featCount} feats, ${classFeatureCount} class features, ${traitCount} traits`);
...
function checkClasses(
  ...
  for (const classData of character.game_character_class) {
    ...
    const entityClass = entity.character.classes.find(c => 
  ...
  console.log(`  Total level: DB=${character.game_character_class.reduce((sum, c) => sum + (c.level || 0), 0)}, Assembled=${assembled.totalLevel}`);
}
