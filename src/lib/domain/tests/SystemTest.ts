...
async function testCapabilities(app: any) {
	...
	if (bonusCapability) {
		...
		typedBonusCapability.addBonus(testEntity, 'strength', 2, 'enhancement', 'test');

		typedBonusCapability.addBonus(testEntity, 'strength', 4, 'morale', 'test');

		// Get calculated value
		const calculatedValue = typedBonusCapability.calculateTotal(testEntity, 'strength');
		console.log(
			`Calculated strength: ${calculatedValue} (Base: ${testEntity.properties.abilities.strength})`
		);
