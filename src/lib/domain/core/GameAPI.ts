...
export class GameAPI {
  ...
  private async initializeCharacterComponents(entity: Entity): Promise<void> {
    ...
    if (abilityCapability) {
      ...
      if (rawData) {
        if (rawData.strength) abilityCapability.setAbilityScore(entity, 'strength', rawData.strength);
        if (rawData.dexterity) abilityCapability.setAbilityScore(entity, 'dexterity', rawData.dexterity);
        if (rawData.constitution) abilityCapability.setAbilityScore(entity, 'constitution', rawData.constitution);
        if (rawData.intelligence) abilityCapability.setAbilityScore(entity, 'intelligence', rawData.intelligence);
        if (rawData.wisdom) abilityCapability.setAbilityScore(entity, 'wisdom', rawData.wisdom);
        if (rawData.charisma) abilityCapability.setAbilityScore(entity, 'charisma', rawData.charisma);
      }
    }
