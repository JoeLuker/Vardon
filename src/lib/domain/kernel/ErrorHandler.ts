...
export function processErrorCode<T>(
	...
	return failure(code, getErrorDescription(code), context);
}
...
export async function withResource<T, R>(
	...
	try {
		...
		return failure(ErrorCode.EIO, error instanceof Error ? error.message : String(error), {
 ...
 */
export function tryWithResources<T>(fn: () => T, cleanup: () => void): Result<T> {
	try {
		const result = fn();
		return success(result);
	} catch (error) {
	...
	return {
		...
		error(
			...
			if (error && typeof error === 'object' && 'code' in error && 'context' in error) {
				...
				// Unknown error type
				systemError = createError(ErrorCode.EIO, message + (error ? `: ${String(error)}` : ''), {
					component,
					operation: context?.operation || 'unknown',
					...context
				});
