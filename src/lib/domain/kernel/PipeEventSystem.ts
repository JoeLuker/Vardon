...
export class PipeEventSystem implements EventEmitter {
  ...
  private subscriptions: Map<string, { queue: string; event: string; listener: EventListener }> = new Map();
  ...
  on(event: string, listener: EventListener, queue: string = DEFAULT_QUEUE): string {
    ...
    if (this.debug) {
      console.log(`[PipeEventSystem] Added listener for event: ${event} on queue: ${queue}, id: ${id}`);
    }
