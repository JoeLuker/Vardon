...
export class MessageQueue {
  ...
  public purgeExpired(): number {
    ...
    this.queue = this.queue.filter(message => {
      const isExpired = message.ttl && message.ttl > 0 && 
                        (now - message.timestamp) > message.ttl;
  ...
  public enqueue(
    ...
    if (this.queue.length >= this.attributes.maxMessages) {
      if (this.attributes.rejectWhenFull) {
        ...
        if (removed) {
          this.log(`Dropped oldest message of type "${removed.type}" to make room in queue "${this.name}"`);
          this.droppedCount++;
        }
      }
    }
  ...
  public dequeue(selector?: MessageSelector): Message | null {
    ...
    this.log(`Dequeued message of type "${message.type}" with id "${message.id}" from queue "${this.name}"`);
