...
export class FileSystem {
		...
		try {
			...
			if (storedMetadata) {
				try {
					...
					if (this.debug)
						console.error('[FileSystem] Failed to parse metadata, initializing fresh filesystem');
	...
	async create(path: string, data: any = null, createParent: boolean = true): Promise<PathResult> {
		...
		if (existingInode) {
			...
			if (existingInode.type === FileType.FILE) {
				...
				this.saveToStorage().catch((err) => {
					if (this.debug)
						console.error(`[FileSystem] Failed to save after updating file: ${normalized}`, err);
				});
		...
		this.saveToStorage().catch((err) => {
			if (this.debug)
				console.error(`[FileSystem] Failed to save after creating file: ${normalized}`, err);
		});
	...
	write(path: string, data: any): ErrorCode {
		...
		if (inode.type === FileType.DEVICE) {
			...
			this.saveToStorage().catch((err) => {
				if (this.debug)
					console.error(`[FileSystem] Failed to save after writing to device: ${normalized}`, err);
			});
		...
		this.saveToStorage().catch((err) => {
			if (this.debug)
				console.error(`[FileSystem] Failed to save after writing to file: ${normalized}`, err);
		});
	...
	unlink(path: string): ErrorCode {
		...
		this.saveToStorage().catch((err) => {
			if (this.debug)
				console.error(`[FileSystem] Failed to save after unlinking file: ${normalized}`, err);
		});
	...
	async mount(path: string, deviceId: string): Promise<PathResult> {
		...
		if (!this.exists(normalized)) {
			...
			this.saveToStorage().catch((err) => {
				if (this.debug)
					console.error(`[FileSystem] Failed to save after mounting device: ${normalized}`, err);
			});
			...
			this.saveToStorage().catch((err) => {
				if (this.debug)
					console.error(
						`[FileSystem] Failed to save after updating device mount: ${normalized}`,
						err
					);
			});
	...
	async umount(path: string): Promise<PathResult> {
		...
		this.saveToStorage().catch((err) => {
			if (this.debug)
				console.error(`[FileSystem] Failed to save after unmounting device: ${normalized}`, err);
		});
	...
	checkFilesystemConsistency(): void {
		...
		for (const [mountPath, deviceId] of this.mountPoints) {
			...
			this.invariants.check(inode !== undefined, `Mount point ${mountPath} has no inode`, {
				...context,
				path: mountPath,
				entity: deviceId
			});
		...
		this.invariants.check(this.inodes.has('/'), 'Root directory inode missing', context);

		// Check root directory entries exist
		this.invariants.check(this.directories.has('/'), 'Root directory entries missing', context);
	}
}
