 ...
 */
export async function initializeApplication(options: { debug?: boolean } = {}): Promise<Application> {
  ...
  async function loadCharacter(characterId: number): Promise<Entity | null> {
    ...
    try {
      ...
      if (entityFd < 0) {
        console.error(`Failed to open entity file after initialization: ${entityPath} (error: ${entityFd})`);
        return null;
      }
 ...
 */
function createUnixDbDevice(gameRulesAPI: GameRulesAPI, options: { debug?: boolean } = {}): Capability {
  ...
  return {
    ...
    ioctl(fd: number, request: number, arg: any): number {
      ...
      if (request === REQUEST.GET_CHARACTER) {
        ...
        this.gameRulesAPI.getCompleteCharacterData(characterId)
        ...
        try {
          ...
          const skills: Record<string, any> = {
            ...
            arcana: {
              ...
              modifiers: [
                { source: 'Intelligence', value: abilities.intelligence?.modifier || 0 }
              ]
            },
            athletics: {
              label: 'Athletics',
  ...
  return {
    ...
    ioctl(fd: number, request: number, arg: any): number {
      ...
      if (request === REQUEST.INITIALIZE) {
        ...
        const entityFd = this.kernel.open(entityPath, OpenMode.READ_WRITE);
        if (entityFd < 0) {
          console.error(`[condition] Failed to open entity file: ${entityPath} (error: ${entityFd})`);
          return ErrorCode.ENOENT;
        }
