...
export class SkillFocusPlugin extends FeatPlugin {
	...
	apply(
		...
		skillCapability.applySkillBonus(entity, skillId, bonusValue, 'competence', source);

		this.log(
			`Applied Skill Focus to skill ${skillId} with a +${bonusValue} bonus for entity ${entity.id}`
		);
	 ...
	 */
	remove(entity: Entity, capabilities: Record<string, Capability>): { removed: boolean } {
		// Get the skill capability
		const skillCapability = capabilities.skill as SkillCapability;
		if (!skillCapability) {
			throw new Error('Skill capability not available');
