...
export function createPipeline(
	options: Omit<PluginProcessOptions, 'execute'>,
	operations: Array<
		(entity: Entity, context: PluginContext, opts?: Record<string, any>) => Promise<Entity>
	>
): Plugin {
	return createFileProcessor(options, async (entity, context, opts) => {
 ...
 */
export interface CapabilityWrapperOptions
	extends Omit<PluginProcessOptions, 'execute' | 'requiredDevices'> {
	/**
...
export function createCapabilityWrapper(options: CapabilityWrapperOptions): Plugin {
	...
	const requiredDevices = [options.capabilityPath, ...(options.additionalDevices || [])];
