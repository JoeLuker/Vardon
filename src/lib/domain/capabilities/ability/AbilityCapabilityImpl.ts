...
export function createUnixAbilityDevice(
   ...
   * @returns ErrorCode (0 for success)
   */
  function readEntityAbility(entityId: string, abilityName: string | undefined, buffer: any): number {
    if (!state.kernel) {
   ...
   * @returns ErrorCode (0 for success)
   */
  function writeEntityAbility(entityId: string, abilityName: string | undefined, buffer: any): number {
    if (!state.kernel) {
      logger.error('Kernel not available', null, {
  ...
  function applyAbilityBonus(arg: any): number {
    ...
    if (!arg || !arg.entityPath || !arg.ability || arg.value === undefined || !arg.type || !arg.source) {
   ...
   * @param source Bonus source
   */
  function removeAbilityBonusFromEntity(
    entity: Entity,
    ability: string,
    source: string
  ): void {
