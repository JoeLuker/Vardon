...
export function createConditionCapability(
	...
	const enhancedCapability = Object.assign(capability, {
		...
		getAllConditions: (entityId: string) => getAllConditions(context, entityId),
		...
		getConditionDefinition: (conditionId: string) => getConditionDefinition(context, conditionId),
		...
		if (existingCondition) {
			if (definition.stacks && options.addStack) {
				...
				if (currentStacks < maxStacks) {
					...
					log(
						context,
						`Cannot add stack to condition ${conditionId}: already at max stacks (${maxStacks})`
					);
					return existingCondition;
				}
		...
		for (const condition of [...entity.properties.conditions]) {
			if (condition.duration !== undefined) {
				...
				if (condition.duration <= 0) {
					...
					removeConditionEffects(context, entityId, condition);
					entity.properties.conditions = entity.properties.conditions.filter(
						(c) => c.id !== condition.id
					);
...
function registerStandardConditions(context: any): void {
	...
	registerConditionDefinition(context, 'blinded', {
		...
		name: 'Blinded',
		description:
			'The character cannot see, taking a -2 penalty to AC, losing Dexterity bonus to AC, and taking a -4 penalty on most Strength and Dexterity-based skill checks and on opposed Perception skill checks.',
		severity: 4,
		removable: true,
		stacks: false,
		source: 'system'
	...
	registerConditionDefinition(context, 'dazzled', {
		...
		name: 'Dazzled',
		description:
			'The creature is unable to see well because of overstimulation of the eyes, taking a -1 penalty on attack rolls and sight-based Perception checks.',
		severity: 1,
		removable: true,
		stacks: false,
		source: 'system'
	...
	registerConditionDefinition(context, 'deafened', {
		...
		name: 'Deafened',
		description:
			'The character cannot hear, taking a -4 penalty on initiative checks and a -4 penalty on Perception checks based on sound.',
		severity: 2,
		removable: true,
		stacks: false,
		source: 'system'
	...
	registerConditionDefinition(context, 'fatigued', {
		...
		name: 'Fatigued',
		description:
			'A fatigued character cannot run or charge and takes a -2 penalty to Strength and Dexterity.',
		severity: 2,
		removable: true,
		stacks: false,
		source: 'system'
	...
	registerConditionDefinition(context, 'frightened', {
		...
		name: 'Frightened',
		description:
			'A frightened creature flees from the source of its fear as best it can, taking a -2 penalty on attack rolls, saving throws, skill checks, and ability checks.',
		severity: 3,
		removable: true,
		stacks: false,
		source: 'system'
	...
	registerConditionDefinition(context, 'stunned', {
		...
		name: 'Stunned',
		description:
			'A stunned creature drops everything held, cannot take actions, takes a -2 penalty to AC, and loses Dexterity bonus to AC.',
		severity: 4,
		removable: true,
		stacks: false,
		source: 'system'
...
function handleIoctl(fd: number, request: number, arg: any, context: any): number {
	...
	if (arg && arg.operation === 'applyCondition' && arg.entityPath && arg.conditionId) {
		return handleApplyConditionOperation(
			arg.entityPath,
			arg.conditionId,
			arg.options || {},
			context
 ...
 * @returns Error code
 */
function handleRemoveConditionOperation(
	entityPath: string,
	conditionId: string,
	context: any
): number {
 ...
 * @returns Error code
 */
function handleProcessExpirationsOperation(
	entityPath: string,
	rounds: number,
	context: any
): number {
		...
		try {
			...
			if (buffer.applyCondition && buffer.applyCondition.id) {
				...
				if (condition) {
					...
					// Remove condition from entity
					entity.properties.conditions = entity.properties.conditions.filter(
						(c) => c.id !== conditionIdToRemove
					);
				}
			} else if (
				buffer.processExpirations &&
				typeof buffer.processExpirations.rounds === 'number'
			) {
				// Process expirations
