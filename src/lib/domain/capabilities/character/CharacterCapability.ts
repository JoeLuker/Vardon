...
export class CharacterCapability implements Capability {
  ...
  onMount(kernel: any): void {
    ...
    if (!this.databaseDriver && kernel) {
      ...
      if (!attachDatabaseDriver()) {
        console.warn(`[CharacterCapability] Database driver not found in kernel during mount, waiting for database:ready event`);
        ...
        if (kernel.events) {
          dbReadySubscription = kernel.events.on('database:ready', (event) => {
            ...
            if (attachDatabaseDriver()) {
              console.log(`[CharacterCapability] Successfully attached database driver after ready event`);
            } else {
              console.error(`[CharacterCapability] Failed to attach database driver after ready event`);
            }
          });
        }
      }
  ...
  async ioctl(fd: number, request: number, arg: any): Promise<number> {
    ...
    if (!this.databaseDriver) {
      ...
      if (this.kernel) {
        const dbDevice = this.kernel.mountPoints?.get('/dev/db') ||
                         this.kernel.devices?.get('/dev/db');
        if (dbDevice) {
          console.log(`[CharacterCapability] Found database driver in kernel, attaching it during ioctl`);
          ...
          const driverDetails = {
            ...
            hasKernel: !!(this.databaseDriver as any).kernel,
            type: this.databaseDriver.constructor ? this.databaseDriver.constructor.name : 'unknown',
            methods: Object.getOwnPropertyNames((this.databaseDriver as any).__proto__).filter(m => typeof (this.databaseDriver as any)[m] === 'function')
          };
          console.log(`[CharacterCapability] Attached database driver details during ioctl:`, driverDetails);
          ...
          console.warn(`[CharacterCapability] Database driver not found in kernel during ioctl, will retry up to 3 times`);
          ...
          while (retries < MAX_RETRIES && !dbDriverFound) {
            retries++;
            console.log(`[CharacterCapability] Retry ${retries}/${MAX_RETRIES} to find database driver`);
          ...
          if (!dbDriverFound) {
            console.error(`[CharacterCapability] Database driver not found after ${MAX_RETRIES} retries`);
            if (arg) {
              arg.errorDetails = {
  ...
  private async getCharacter(arg: any): Promise<number> {
    ...
    try {
      ...
      if (!this.databaseDriver) {
        ...
        if (this.kernel) {
          console.log(`[CharacterCapability] Attempting last-ditch database driver recovery in getCharacter`);
      ...
      while (retryCount < MAX_RETRIES) {
        try {
          console.log(`[CharacterCapability] Attempt ${retryCount + 1}/${MAX_RETRIES} to get character ${characterId}`);
          ...
          if (characterData && characterData.id) {
            ...
            console.error(`[CharacterCapability] Invalid character data received (attempt ${retryCount + 1})`);
            retryCount++;
          ...
          lastError = dbError;
          console.error(`[CharacterCapability] Database error getting character (attempt ${retryCount + 1}):`, dbError);
      ...
      console.error(`[CharacterCapability] All ${MAX_RETRIES} attempts to fetch character ${characterId} failed`);
