 ...
 */
export function createBonusCapability(
  options: BonusCapabilityOptions = {}
): BonusCapability {
  ...
  const enhancedCapability = Object.assign(capability, {
    ...
    calculateTotal: (entity: Entity, target: string) => 
      calculateTotal(context, entity, target),
      
    getBreakdown: (entity: Entity, target: string) => 
      getBreakdown(context, entity, target),
      
    hasBonus: (entity: Entity, target: string, source: string) => 
      hasBonus(context, entity, target, source),
      
    getAllBonuses: (entity: Entity) => 
      getAllBonuses(context, entity),
      
    getComponents: (entity: Entity, target: string) => 
      getComponents(context, entity, target),
...
function calculateTotal(context: any, entity: Entity, target: string): number {
  ...
  for (const type in bonusesByType) {
    ...
    const shouldStack = context.stackSameType || 
                        context.stackingTypes.includes(type);
...
function handleIoctl(fd: number, request: number, arg: any, context: any): number {
  ...
  if (arg && arg.operation === 'addBonus' && 
      arg.entityPath && arg.target && arg.value !== undefined && arg.type && arg.source) {
    return handleAddBonusOperation(arg.entityPath, arg.target, arg.value, arg.type, arg.source, context);
  }
  
  if (arg && arg.operation === 'removeBonus' && 
      arg.entityPath && arg.target && arg.source) {
