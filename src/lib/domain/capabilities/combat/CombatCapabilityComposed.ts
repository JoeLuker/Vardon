...
export function createCombatCapability(
	...
	return Object.assign(capability, {
		...
		getCombatStats: (entity: Entity) => getCombatStats(context, entity),
...
function getCombatStats(context: CombatCapabilityContext, entity: Entity): CombatStats {
	...
	if (
		...
		Array.isArray(entity.properties.combat.meleeBonuses)
	) {
	...
	if (
		...
		Array.isArray(entity.properties.combat.rangedBonuses)
	) {
	...
	if (
		entity.properties.combat?.saves?.fortitude?.bonuses &&
	...
	// Add reflex bonuses
	if (
		entity.properties.combat?.saves?.reflex?.bonuses &&
	...
	// Add will bonuses
	if (
		entity.properties.combat?.saves?.will?.bonuses &&
...
function addBaseAttackBonusModifier(
	...
	context.bonusCapability.addBonus(entity, 'base_attack_bonus', value, source);
...
function addACModifier(
	...
	const appliesToFlatFooted =
		options.appliesToFlatFooted !== undefined ? options.appliesToFlatFooted : true;
