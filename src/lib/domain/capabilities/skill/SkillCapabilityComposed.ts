...
export function createSkillCapability(
  ...
  return Object.assign(capability, {
    ...
    // Get all skills
    getAllSkills: (entity: Entity) => 
      getAllSkills(context, entity)
  });
}
...
function setClassSkill(
  ...
  log(context, `Set class skill status for skill ${skillId} to ${isClassSkillValue} for entity ${entity.id}`);
}
...
function applySkillBonus(
  ...
  try {
    ...
    log(context, `Applied ${type} bonus of ${value} to skill ${skillId} for entity ${entity.id} from ${source}`);
  } catch (err) {
...
function handleIoctl(fd: number, request: number, arg: any, context: SkillCapabilityContext): number {
  ...
  if (arg && arg.operation === 'getSkillAvailable' && 
      arg.entityPath && typeof arg.level === 'number') {
    return handleGetAvailableSkillPoints(arg.entityPath, arg.level, context);
  }
  
  if (arg && arg.operation === 'getSkillInvested' && 
      arg.entityPath && arg.level !== undefined) {
