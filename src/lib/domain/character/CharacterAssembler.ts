...
export class UnixCharacterAssembler {
   ...
   */
  public async assembleCharacter(characterData: CompleteCharacter): Promise<Result<AssembledCharacter>> {
      ...
      return withFile(
        ...
        (fd) => {
          ...
          if (writeResult !== ErrorCode.SUCCESS) {
            return failure(
              writeResult,
              `Failed to update entity: ${entityPath}`,
              {
                component: 'CharacterAssembler',
      ...
      const result = await withFile(
        ...
        (fd) => {
          ...
          if (ioctlResult !== ErrorCode.SUCCESS) {
            return failure(
              ioctlResult,
              `Failed to initialize capability: ${capability.name}`,
              {
                component: 'CharacterAssembler',
   ...
   * @returns Success or failure result
   */
  private async applyFeatures(entityPath: string, characterData: CompleteCharacter): Promise<Result<void>> {
    ...
    const featureCategories = [
      ...
      {
        name: 'corruption',
        items: characterData.game_character_corruption_manifestation?.map(x => x.manifestation) || [],
   ...
   */
  private async applyFallbackBonuses(entityPath: string, feature: any, featureType: string): Promise<void> {
    const bonuses = [];
   ...
   * @param characterData Raw character data
   */
  private async applyEquipmentEffects(entityPath: string, characterData: CompleteCharacter): Promise<void> {
    // Apply armor bonuses
   ...
   */
  private async calculateCharacterValues(entityPath: string, characterData: CompleteCharacter): Promise<Result<AssembledCharacter>> {
    try {
      // Get entity data
      const entityResult = await withFile(
        this.kernel,
        entityPath,
        OpenMode.READ,
        (fd) => {
      ...
      const totalLevel = characterData.game_character_class?.reduce(
        (sum, classEntry) => sum + (classEntry.level || 0), 0
      ) || 0;
      ...
      return success({
        ...
        conMod: abilities.constitution.total ? Math.floor((abilities.constitution.total - 10) / 2) : 0,
        intMod: abilities.intelligence.total ? Math.floor((abilities.intelligence.total - 10) / 2) : 0,
        ...
        abpData: {
          ...
          appliedBonuses: entity.properties.bonuses ? 
            Object.entries(entity.properties.bonuses)
          ...
          for (const abilityName of Object.keys(abilities)) {
            ...
            if (breakdown) {
              abilities[abilityName] = {
                ...
                modifiers: breakdown.bonuses?.components?.map(c => ({ 
                  source: c.source, 
                  value: c.value 
                })) || [],
  ...
  private async calculateCombatStats(entityPath: string): Promise<any> {
    const result = {
      ...
      attacks: {
        ...
        bomb: { label: 'Bomb Attack', attack: { label: 'Bomb Attack', modifiers: [], total: 0 }, damage: { label: 'Bomb Damage', modifiers: [], total: 0 }, bombDice: 0 }
      }
    };
        ...
        (fd) => {
            ...
            if (ioctlResult === ErrorCode.SUCCESS) {
              ...
              if (readResult === ErrorCode.SUCCESS && breakdown) {
                result.saves[saveType] = {
                  ...
                  modifiers: breakdown.otherBonuses?.components?.map(c => ({ 
                    source: c.source, 
                    value: c.value 
                  })) || [],
          ...
          if (acIoctlResult === ErrorCode.SUCCESS) {
            ...
            if (readResult === ErrorCode.SUCCESS && acBreakdown) {
              ...
              result.ac = {
                label: 'AC',
                modifiers: acBreakdown.otherBonuses?.components?.map(c => ({ 
                  source: c.source, 
                  value: c.value 
                })) || [],
   ...
   * @returns Skill data
   */
  private async calculateSkills(entityPath: string, characterData: CompleteCharacter): Promise<any> {
    const result = {
      skills: {},
      skillsWithRanks: []
    };
          ...
          if (allSkills) {
            for (const [skillId, skill] of Object.entries(allSkills)) {
              result.skills[skillId] = {
                ...
                modifiers: skill.otherBonuses?.components?.map(c => ({ 
                  source: c.source, 
                  value: c.value 
                })) || [],
                total: skill.total || 0,
