...
registerTestSuite({
	...
	run: async () => {
		...
		await ctx.asyncTest('Character level progression', async () => {
			...
			ctx.assertEquals(
				loaded!.properties.skills.knowledge_arcana.ranks,
				2,
				'Skill ranks should increase'
			);
		...
		await ctx.asyncTest('Apply feats through plugins', async () => {
			...
			ctx.assertEquals(
				loaded!.properties.skills.perception.miscBonus,
				3,
				'Perception should have +3'
			);
		...
		await ctx.asyncTest('Spellcaster spell management', async () => {
			...
			ctx.assertEquals(
				loaded!.properties.spells.slots[1].used,
				2,
				'Should have used 2 1st level slots'
			);
			ctx.assertEquals(
				loaded!.properties.spells.slots[1].total - loaded!.properties.spells.slots[1].used,
				2,
				'Should have 2 1st level slots remaining'
			);
		...
		await ctx.asyncTest('Full system integration stress test', async () => {
			...
			for (const character of party) {
				...
				ctx.assertLessThan(
					loaded!.properties.hp.current,
					loaded!.properties.hp.max,
					'Should be damaged'
				);
			}
