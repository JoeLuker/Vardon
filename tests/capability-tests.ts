...
registerTestSuite({
  ...
  run: async () => {
    ...
    await ctx.asyncTest('Capability operation performance', async () => {
      ...
      ctx.assertEquals(cap.operations.filter(op => op.startsWith('read:')).length, iterations, 'Should track all reads');
      ctx.assertEquals(cap.operations.filter(op => op.startsWith('write:')).length, iterations, 'Should track all writes');
