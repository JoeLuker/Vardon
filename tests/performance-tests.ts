...
registerTestSuite({
	...
	run: async () => {
		...
		await ctx.asyncTest('Filesystem operation throughput', async () => {
			...
			console.log(
				`  FS Performance: Create=${createOps.toFixed(0)}/s, Read=${readOps.toFixed(0)}/s, Delete=${deleteOps.toFixed(0)}/s`
			);
		});
		...
		await ctx.asyncTest('Character creation and loading', async () => {
			...
			console.log(
				`  Character Performance: Create=${createRate.toFixed(1)}/s, Load=${loadRate.toFixed(0)}/s`
			);
		...
		await ctx.asyncTest('Concurrent operation performance', async () => {
			...
			console.log(
				`  Concurrent Performance: ${opsPerSecond.toFixed(0)} ops/s with ${concurrency} workers`
			);
