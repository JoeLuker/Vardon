...
test.describe('Character Sheet Interactive E2E Tests', () => {
		...
		test('should update current HP when damaged', async ({ page }) => {
			...
			if ((await hpTracker.count()) > 0) {
				...
				const damageInput = page
					.locator('input[placeholder*="damage"], input[type="number"]')
					.first();
				const applyButton = page
					.locator('button:has-text("Apply"), button:has-text("Damage")')
					.first();
	...
	test.describe('Collapsible Sections', () => {
		test('should expand and collapse sections', async ({ page }) => {
			...
			const collapsibleHeaders = page.locator(
				'[aria-expanded], .collapsible-header, .accordion-header'
			);
	...
	test.describe('Character Sheet Actions', () => {
		test('should handle rest actions', async ({ page }) => {
			...
			const restButton = page.locator(
				'button:has-text("Rest"), button:has-text("Long Rest"), button:has-text("Short Rest")'
			);
